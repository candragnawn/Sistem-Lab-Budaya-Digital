...
const pejabatData = [
  { id: 1, jabatan: "Kepala Laboratorium Warisan Budaya Digital", unitKerja: "Universitas Udayana", noSK: "SK/224/UN.14/2020", tmt: "1 Januari 2020", sampai: "31 Desember 2024", status: "Selesai" },
  { id: 2, jabatan: "Ketua Pusat Kajian Budaya Bali", unitKerja: "Universitas Udayana", noSK: "SK/389/UN.14/2023", tmt: "1 Januari 2023", sampai: "31 Desember 2027", status: "Aktif" },
];
...
export default function PejabatPage() {
  ...
  if (!user) return null;
  const filteredData = pejabatData.filter(i => i.jabatan.toLowerCase().includes(searchQuery.toLowerCase()) || i.unitKerja.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl bg-white p-6 shadow-sm border border-gray-100">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex gap-2">
            <Button size="sm" className="bg-brand-navy text-white hover:bg-brand-navy/90 text-xs gap-1.5"><RefreshCw className="h-3 w-3" /> Sync SISTER</Button>
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="text-brand-navy font-medium">Profil</span><ChevronRight className="h-3 w-3 text-gray-400" /><span className="text-brand-navy/80 font-medium">Pejabat Struktural</span>
      </div>
      <div className="rounded-xl bg-white shadow-sm border border-gray-100 overflow-hidden">
        <div className="bg-gradient-to-r from-brand-navy/5 to-transparent p-6 border-b border-gray-100">
          <div className="flex items-start justify-between">
            <div><p className="text-xs font-medium uppercase tracking-wider text-amber-600/70 mb-1">PROFIL · SISTER</p><h2 className="text-2xl font-medium text-gray-600">Pejabat Struktural</h2><p className="mt-1.5 text-sm text-gray-500">Riwayat jabatan struktural yang pernah atau sedang diemban.</p></div>
        ...
        <div className="p-6">
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-200">
            <table className="w-full text-sm" id="pejabat-table">
              <thead><tr className="border-b border-gray-200 bg-gray-50/80">
                ...
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-400">Nama Jabatan</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredData.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-4 text-gray-600 font-mono text-xs">{item.noSK}</td>
                    ...
                    <td className="px-4 py-4"><div className="flex items-center justify-center gap-1">
                      <button className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-blue-50 hover:text-blue-600" id={`view-pejabat-${item.id}`}><Eye className="h-4 w-4" /></button>
                      <button className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-yellow-50 hover:text-yellow-600" id={`edit-pejabat-${item.id}`}><Pencil className="h-4 w-4" /></button>
                      <button className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-600" id={`delete-pejabat-${item.id}`}><Trash2 className="h-4 w-4" /></button>
                    </div></td>
                  </tr>
                ))}
          ...
          <div className="mt-4 flex items-center justify-between">
            <p className="text-xs text-gray-500">Menampilkan {filteredData.length} dari {pejabatData.length} data</p>
