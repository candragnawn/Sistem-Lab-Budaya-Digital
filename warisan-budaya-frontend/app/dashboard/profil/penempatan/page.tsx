...
import { CheckCircle, Download, RefreshCw, Plus, Search, Eye, Pencil, Trash2, Home, ChevronRight, MapPin } from "lucide-react";

interface UserData { name: string; role: string; photo: string; }

const penempatanData = [
  { id: 1, unitKerja: "Laboratorium Warisan Budaya Digital", perguruan: "Universitas Udayana", jabatan: "Dosen Tetap", tmt: "1 Maret 1993", status: "Aktif" },
  { id: 2, unitKerja: "Pusat Kajian Budaya Bali", perguruan: "Universitas Udayana", jabatan: "Kepala Pusat Kajian", tmt: "1 Januari 2020", status: "Aktif" },
];
...
export default function PenempatanPage() {
  ...
  if (!user) return null;
  const filteredData = penempatanData.filter(i => i.unitKerja.toLowerCase().includes(searchQuery.toLowerCase()) || i.jabatan.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <div className="rounded-xl bg-white shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
            <div className="relative max-w-xs"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" /><Input placeholder="Cari unit kerja..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 h-10 text-sm border-gray-200 rounded-lg" /></div>
            <div className="flex gap-2">
              ...
              <tbody className="divide-y divide-gray-100">
                {filteredData.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-4 font-medium text-gray-800">{item.unitKerja}</td>
                    <td className="px-4 py-4 text-gray-600">{item.perguruan}</td>
                    ...
                    <td className="px-4 py-4"><div className="flex items-center justify-center gap-1">
                      <button className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-blue-50 hover:text-blue-600" id={`view-penempatan-${item.id}`}><Eye className="h-4 w-4" /></button>
                      <button className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-yellow-50 hover:text-yellow-600" id={`edit-penempatan-${item.id}`}><Pencil className="h-4 w-4" /></button>
                      <button className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-600" id={`delete-penempatan-${item.id}`}><Trash2 className="h-4 w-4" /></button>
                    </div></td>
                  </tr>
                ))}
          ...
          <div className="mt-4 flex items-center justify-between">
            <p className="text-xs text-gray-500">Menampilkan {filteredData.length} dari {penempatanData.length} data</p>
