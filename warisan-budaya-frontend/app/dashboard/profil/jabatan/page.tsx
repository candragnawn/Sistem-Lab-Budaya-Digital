...
const jabatanData = [
  { id: 1, jabatan: "Asisten Ahli", pangkat: "Penata Muda / III-a", angkaKredit: "100", tmt: "1 Maret 2000", nomorSK: "12345/A/2000", status: "Selesai" },
  { id: 2, jabatan: "Lektor", pangkat: "Penata / III-c", angkaKredit: "200", tmt: "1 September 2004", nomorSK: "23456/B/2004", status: "Selesai" },
  { id: 3, jabatan: "Lektor Kepala", pangkat: "Pembina / IV-a", angkaKredit: "400", tmt: "1 Oktober 2012", nomorSK: "48763/C/2012", status: "Selesai" },
  { id: 4, jabatan: "Guru Besar", pangkat: "Pembina Utama Muda / IV-c", angkaKredit: "850", tmt: "1 Januari 2018", nomorSK: "90124/D/2018", status: "Aktif" },
];
...
export default function JabatanPage() {
  ...
  if (!user) return null;
  const filteredData = jabatanData.filter(i => i.jabatan.toLowerCase().includes(searchQuery.toLowerCase()) || i.pangkat.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                <span className="inline-flex items-center rounded-full bg-slate-100 px-2.5 py-0.5 text-[10px] font-medium text-slate-600">NIDN: 0001196801</span>
              </div>
            </div>
          </div>
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="text-brand-navy font-medium">Profil</span><ChevronRight className="h-3 w-3 text-gray-400" /><span className="text-brand-navy/80 font-medium">Jabatan Fungsional</span>
      </div>
      <div className="rounded-xl bg-white shadow-sm border border-gray-100 overflow-hidden">
        <div className="bg-gradient-to-r from-brand-navy/5 to-transparent p-6 border-b border-gray-100">
          <div className="flex items-start justify-between">
            <div><p className="text-xs font-medium uppercase tracking-wider text-amber-600/70 mb-1">PROFIL</p><h2 className="text-2xl font-medium text-gray-600">Jabatan Fungsional</h2><p className="mt-1.5 text-sm text-gray-500">Riwayat jabatan fungsional akademik dosen dari awal hingga saat ini.</p></div>
        ...
        <div className="p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-6">
            <div className="relative max-w-xs"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" /><Input placeholder="Cari jabatan..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9 h-10 text-sm border-gray-200 rounded-lg" /></div>
            <div className="flex gap-2">
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-200">
            <table className="w-full text-sm" id="jabatan-table">
              <thead><tr className="border-b border-gray-200 bg-gray-50/80">
                ...
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-400">Pangkat / Gol</th>
                <th className="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-400">AK</th>
                ...
                {filteredData.map((item, idx) => (
                  <tr key={item.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-4 py-4 text-center">
                      <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${item.status === "Aktif" ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-600"}`}>{item.status}</span>
                    </td>
                    <td className="px-4 py-4"><div className="flex items-center justify-center gap-1">
                      <button className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-blue-50 hover:text-blue-600" id={`view-jabatan-${item.id}`}><Eye className="h-4 w-4" /></button>
                      <button className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-yellow-50 hover:text-yellow-600" id={`edit-jabatan-${item.id}`}><Pencil className="h-4 w-4" /></button>
                      <button className="flex h-8 w-8 items-center justify-center rounded-lg text-gray-400 hover:bg-red-50 hover:text-red-600" id={`delete-jabatan-${item.id}`}><Trash2 className="h-4 w-4" /></button>
                    </div></td>
                  </tr>
                ))}
          ...
          <div className="mt-4 flex items-center justify-between">
            <p className="text-xs text-gray-500">Menampilkan {filteredData.length} dari {jabatanData.length} data</p>
