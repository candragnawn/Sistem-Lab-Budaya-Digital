...
export default function BimbinganPage() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const res = await api.get("/student-supervisions");
      ...
      toast.error("Gagal mengambil data Bimbingan Mahasiswa");
      setData([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        supervision_type: formData.jenis_bimbingan,
        study_program: formData.program_studi,
      };

      if (formData.id) {
        await api.put(`/supervisions/${formData.id}`, payload);
        toast.success("Data Bimbingan berhasil diubah");
      } else {
        await api.post("/supervisions", payload);
  ...
  const filteredData = data.filter((i: any) => {
    ...
    return (i.kategori_kegiatan && String(i.kategori_kegiatan).toLowerCase().includes(q)) || (i.jenis_bimbingan && String(i.jenis_bimbingan).toLowerCase().includes(q)) || (i.program_studi && String(i.program_studi).toLowerCase().includes(q)) || false;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="text-brand-navy font-medium">Pelaksanaan Pendidikan</span><ChevronRight className="h-3 w-3 text-gray-400" /><span className="text-brand-navy/80 font-medium">Bimbingan Mahasiswa</span>
      </div>
      ...
      <div className="rounded-xl bg-white shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-lg font-semibold text-gray-800">Data Bimbingan Mahasiswa</h2>
          ...
          <div className="w-full max-w-2xl bg-white rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[95vh]">
            <div className="bg-brand-navy p-6 pt-8 text-white relative flex-shrink-0">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-white text-xl font-semibold m-0">{formData.id ? "Edit Data Bimbingan" : "Tambah Data Bimbingan"}</h2>
                  <p className="text-gray-300 mt-1 text-sm">Lengkapi detail kegiatan bimbingan Anda.</p>
                </div>
              </div>
            </div>
