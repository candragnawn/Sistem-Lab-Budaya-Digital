...
export default function DosenProfilePage() {
  ...
  const isOwner = true; // Dashboard management mode
  ...
  return (
    <div className="bg-[#F5F5F5] relative pb-20 -mx-4 -mt-4 lg:-mx-8 lg:-mt-8">
      {/* Header top banner removed for dashboard */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 pt-8">
        ...
        {activeTab !== "overview" && activeTab !== "publications" && activeCategory && (
          <div className="animate-[fadeIn_0.3s_ease]">
            {isOwner && (
              <div className="mb-5 flex flex-col sm:flex-row items-start sm:items-center gap-3 bg-brand-navy/5 border border-brand-navy/20 rounded-xl px-5 py-3.5">
                <div className="flex items-center gap-2 text-brand-navy">
                  <Settings className="w-4 h-4" />
                  <span className="text-sm font-semibold">Mode Pengaturan Visibilitas Publik</span>
                </div>
                <span className="text-xs text-brand-navy/70">
                  Atur visibilitas tabel sub-kategori dengan tombol <Eye className="inline w-3.5 h-3.5 mx-0.5" /> / <EyeOff className="inline w-3.5 h-3.5 mx-0.5" />.
                  Tabel yang disembunyikan tidak akan muncul di profil publik Anda.
                </span>
              </div>
            )}
