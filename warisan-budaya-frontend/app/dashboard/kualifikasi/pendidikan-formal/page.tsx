...
export default function PendidikanFormalPage() {
  ...
  const fetchData = async () => {
    setIsLoading(true);
    try {
      const res = await api.get("/lecturer-educations");
      ...
      toast.error("Gagal mengambil data Pendidikan Formal");
      setData([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const filteredData = data.filter((i: any) => {
    ...
    return (i.tingkat && String(i.tingkat).toLowerCase().includes(q)) || (i.universitas && String(i.universitas).toLowerCase().includes(q)) || (i.program_studi && String(i.program_studi).toLowerCase().includes(q)) || false;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-2 text-sm">
        ...
        <span className="text-brand-navy font-medium">Kualifikasi</span><ChevronRight className="h-3 w-3 text-gray-400" /><span className="text-brand-navy/80 font-medium">Pendidikan Formal</span>
      </div>
      ...
      <div className="rounded-xl bg-white shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-6 border-b border-gray-100">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h2 className="text-lg font-semibold text-gray-800">Data Pendidikan Formal</h2>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50/80">
                ...
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-400">Jenjang</th>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-400">Tahun Lulus</th>
