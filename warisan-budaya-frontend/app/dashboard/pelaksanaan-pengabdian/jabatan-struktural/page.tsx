...
export default function JabatanStrukturalPage() {
  ...
  const [data, setData] = useState<StructuralPosition[]>([]);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      toast.error("Gagal mengambil data Jabatan Struktural");
      setData([]);
    } finally {
      setIsLoading(false);
    }
        ...
        <div className="p-6 border-b border-gray-100">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">{data.length} data jabatan struktural ditemukan</p>
            </div>
            <div className="flex items-center gap-2">
              
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                <Input placeholder="Cari jabatan atau No SK..." className="pl-9 h-9 text-sm w-64 border-gray-200" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50/80">
                ...
                <th className="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-400">Terhitung Tanggal Mulai</th>
                <th className="px-4 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-400">Terhitung Tanggal Selesai</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredData.map((item, idx) => (
                <tr key={item.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-4 align-top font-medium text-gray-800">{item.jabatan_struktural}</td>
                  <td className="px-4 py-4 align-top text-gray-600">{item.nomor_sk}</td>
                  <td className="px-4 py-4 text-center text-gray-600 align-top font-mono">{item.tanggal_mulai}</td>
