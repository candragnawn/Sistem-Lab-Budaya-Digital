  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-base font-medium text-white">Data Profesor Emeritus</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#111827] h-[36px]">
                <th className="px-3.5 text-left text-[11px] font-medium text-gray-400">Nama & Gelar</th>
                <th className="px-3.5 text-left text-[11px] font-medium text-gray-400">Perguruan Tinggi</th>
                <th className="px-3.5 text-left text-[11px] font-medium text-gray-400">TMT</th>
                <th className="px-3.5 text-left text-[11px] font-medium text-gray-400">TST</th>
              ...
              {mockData.length > 0 ? (
                mockData.map((item, i) => (
                  <tr key={item.id} className={`h-[44px] hover:bg-gray-800 ${i < mockData.length - 1 ? 'border-b border-gray-700' : ''}`}>
                    <td className="px-3.5 text-[11px] text-white">{item.nama_gelar}</td>
                    <td className="px-3.5 text-[11px] text-white">{item.perguruan_tinggi}</td>
                    <td className="px-3.5 text-[11px] text-white">{item.tmt}</td>
                    <td className="px-3.5 text-[11px] text-white">{item.tst || '-'}</td>
                    <td className="px-3.5">
                ...
                <tr>
                  <td colSpan={5} className="px-3.5 py-8 text-center">
                    <p className="text-[11px] italic text-gray-500">(Tidak ada data profesor emeritus)</p>
                  </td>
                </tr>
              )}
            </tbody>
