  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-base font-medium text-white">Data Anggota Profesi</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-[#111827] h-[36px]">
                <th className="px-3.5 text-left text-[11px] font-medium text-gray-400">Nama Organisasi</th>
            ...
            <tbody>
              {mockData.map((item, i) => (
                <tr key={item.id} className={`h-[44px] hover:bg-gray-800 ${i < mockData.length - 1 ? 'border-b border-gray-700' : ''}`}>
                  <td className="px-3.5 text-[11px] text-white">{item.nama_organisasi}</td>
                  <td className="px-3.5 text-[11px] text-white">{item.peran}</td>
                  <td className="px-3.5 text-[11px] text-white">{item.institusi_profesional}</td>
