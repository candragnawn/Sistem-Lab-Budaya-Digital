  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-base font-medium text-brand-card">Data Tes Kompetensi</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-surface-darker h-[36px]">
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Jenis Tes</th>
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Nama Tes</th>
                ...
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Skor/Nilai</th>
            ...
            <tbody>
              {mockData.map((item, i) => (
                <tr key={item.id} className={`h-[44px] hover:bg-gray-800 ${i < mockData.length - 1 ? 'border-b border-border-dark' : ''}`}>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.jenis_tes}</td>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.nama_tes}</td>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.penyelenggara}</td>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.tanggal_tes}</td>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.skor_nilai}</td>
                  <td className="px-3.5">
