...
const mockData: PengabdianData[] = [
  {
    id: 1,
    judul: 'Pelatihan Digitalisasi Dokumen Desa Adat Berbasis Cloud di Buleleng',
    bidang_ilmu: 'Teknologi Informasi & Pemberdayaan Masyarakat',
    tahun_pelaksanaan: '2023',
    durasi: 6,
  },
];

export function Pengabdian() {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-base font-medium text-brand-card">Data Pengabdian Masyarakat</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-surface-darker h-[36px]">
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Judul Kegiatan</th>
                ...
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Durasi (Bulan)</th>
            ...
            <tbody>
              {mockData.map((item, i) => (
                <tr key={item.id} className={`h-[44px] hover:bg-gray-800 ${i < mockData.length - 1 ? 'border-b border-border-dark' : ''}`}>
                  ...
                  <td className="px-3.5 text-[11px] text-brand-card">{item.durasi}</td>
                  <td className="px-3.5">
