...
interface JabatanStrukturalData {
  id: number;
...
const mockData: JabatanStrukturalData[] = [
  {
    id: 1,
    jabatan_struktural: 'Sekretaris Jurusan Teknik Informatika',
    nomor_sk: 'SK-STRUKTUR-2023-04',
    tanggal_mulai: '2023-02-01',
    tanggal_selesai: '2027-02-01',
  },
];

export function JabatanStruktural() {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-base font-medium text-brand-card">Data Jabatan Struktural</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-surface-darker h-[36px]">
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Jabatan Struktural</th>
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Nomor SK</th>
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Tanggal Mulai</th>
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Tanggal Selesai</th>
            ...
            <tbody>
              {mockData.map((item, i) => (
                <tr key={item.id} className={`h-[44px] hover:bg-gray-800 ${i < mockData.length - 1 ? 'border-b border-border-dark' : ''}`}>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.jabatan_struktural}</td>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.nomor_sk}</td>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.tanggal_mulai}</td>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.tanggal_selesai}</td>
                  <td className="px-3.5">
