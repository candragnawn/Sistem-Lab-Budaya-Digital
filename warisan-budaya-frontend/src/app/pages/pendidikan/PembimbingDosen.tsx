...
const mockData: PembimbingDosenData[] = [
  {
    id: 1,
    nama_pembimbing: 'Prof. Dr. I Nyoman Adi, M.T. (Membimbing Dosen Pemula)',
    tanggal_mulai: '2023-09-01',
    tanggal_selesai: '2024-08-31',
  },
];

export function PembimbingDosen() {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-base font-medium text-brand-card">Data Pembimbing Dosen</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-surface-darker h-[36px]">
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Nama Pembimbing</th>
            ...
            <tbody>
              {mockData.map((item, i) => (
                <tr key={item.id} className={`h-[44px] hover:bg-gray-800 ${i < mockData.length - 1 ? 'border-b border-border-dark' : ''}`}>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.nama_pembimbing}</td>
