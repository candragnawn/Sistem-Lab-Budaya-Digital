...
const mockData: PengujianData[] = [
  {
    id: 1,
    judul_ujian: 'Sidang Skripsi Akhir Semester Genap',
    bidang_ilmu: 'Kecerdasan Buatan',
    jenis_ujian: 'Ketua Penguji',
    program_studi: 'Teknik Informatika',
  },
];

export function PengujianMahasiswa() {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-base font-medium text-brand-card">Data Pengujian Mahasiswa</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-surface-darker h-[36px]">
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Judul Ujian</th>
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Bidang Ilmu</th>
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Jenis Ujian</th>
            ...
            <tbody>
              {mockData.map((item, i) => (
                <tr key={item.id} className={`h-[44px] hover:bg-gray-800 ${i < mockData.length - 1 ? 'border-b border-border-dark' : ''}`}>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.judul_ujian}</td>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.bidang_ilmu}</td>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.jenis_ujian}</td>
