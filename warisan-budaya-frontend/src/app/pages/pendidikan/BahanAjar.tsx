...
const mockData: BahanAjarData[] = [
  {
    id: 1,
    judul: 'Buku Ajar: Dasar-Dasar Next.js dan TailwindCSS untuk Pemula',
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-base font-medium text-brand-card">Data Bahan Ajar</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-surface-darker h-[36px]">
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Judul</th>
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">ISBN</th>
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Tanggal Publikasi</th>
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Penerbit</th>
            ...
            <tbody>
              {mockData.map((item, i) => (
                <tr key={item.id} className={`h-[44px] hover:bg-gray-800 ${i < mockData.length - 1 ? 'border-b border-border-dark' : ''}`}>
                  <td className="px-3.5 text-[11px] text-brand-card max-w-[300px] truncate">{item.judul}</td>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.isbn}</td>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.tanggal_publikasi}</td>
                  <td className="px-3.5 text-[11px] text-brand-card">{item.penerbit}</td>
                  <td className="px-3.5">
