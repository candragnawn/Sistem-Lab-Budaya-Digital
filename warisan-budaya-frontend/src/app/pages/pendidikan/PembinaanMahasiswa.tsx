...
const mockData: PembinaanData[] = [
  {
    id: 1,
    semester: '2023 GENAP',
    kategori_kegiatan: 'Pembinaan PKM (Program Kreativitas Mahasiswa)',
    judul_bimbingan: 'Aplikasi Web Pemetaan Warisan Budaya Bali Berbasis GIS',
    jenis_bimbingan: 'Dosen Pembimbing Lapangan',
    program_studi: 'Teknik Informatika',
  },
];

export function PembinaanMahasiswa() {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-base font-medium text-brand-card">Data Pembinaan Mahasiswa</h2>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-surface-darker h-[36px]">
                ...
                <th className="px-3.5 text-left text-[11px] font-medium text-text-placeholder">Judul Bimbingan</th>
            ...
            <tbody>
              {mockData.map((item, i) => (
                <tr key={item.id} className={`h-[44px] hover:bg-gray-800 ${i < mockData.length - 1 ? 'border-b border-border-dark' : ''}`}>
                  ...
                  <td className="px-3.5 text-[11px] text-brand-card max-w-[250px] truncate">{item.judul_bimbingan}</td>
